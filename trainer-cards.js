...
const TRAINER_EFFECTS = {
  ...
  // Opponent shuffles hand into deck and draws 7.
  'Impostor Professor Oak': async ({ player, oppP, consume }) => {
    consume();
    oppP.discard.push(...oppP.hand);
    oppP.hand = [];
    oppP.deck = shuffle(oppP.deck);
  ...
  // ── Potion ────────────────────────────────────────────────────────────────
  // Remove up to 2 damage counters (20 damage) from 1 of your Pokémon.
  ...
  // ── Super Potion ──────────────────────────────────────────────────────────
  // Discard 1 energy from own Pokémon to remove 4 damage counters (40 damage).
  'Super Potion': async ({ player, p, consume }) => {
    ...
    consume();
    p.active.attachedEnergy.splice(0, 1);
  ...
  // ── Defender ─────────────────────────────────────────────────────────────
  // Active Pokémon takes 20 less damage until end of opponent's next turn.
  ...
  // ── Energy Retrieval ──────────────────────────────────────────────────────
  // Trade 1 card from hand for up to 2 basic Energy from discard.
  'Energy Retrieval': async ({ player, p, consume }) => {
    ...
    consume();
    const toGet = basics.slice(0, 2);
    toGet.forEach(e => { const i = p.discard.findIndex(c => c === e); if (i !== -1) p.hand.push(...p.discard.splice(i, 1)); });
    addLog(`P${player} used Energy Retrieval — returned ${toGet.length} energy card(s) to hand.`, true);
    renderAll();
  },
  ...
  'Computer Search': async ({ player, p, card, handIdx }) => {
    ...
    const discardPicked = await openCardPicker({ title: 'Computer Search', subtitle: 'Discard 2 cards from your hand', cards: others, maxSelect: 2 });
    if (!discardPicked || discardPicked.length < 2) { showToast('Computer Search cancelled.'); return; }
  ...
  // ── Lass ──────────────────────────────────────────────────────────────────
  // Both players reveal hands; discard all Trainer cards from both.
  'Lass': async ({ player, p, card, handIdx }) => {
    ...
    for (const pNum of [1, 2]) {
      ...
      trainers.forEach(t => { const i = pp.hand.findIndex(c => c === t); if (i !== -1) pp.discard.push(...pp.hand.splice(i, 1)); });
      discarded[pNum] = trainers.map(t => t.name);
    }
    p.discard.push(card);
    addLog(`P${player} used Lass — P1 discarded: ${discarded[1].join(', ')||'none'}; P2 discarded: ${discarded[2].join(', ')||'none'}.`, true);
  ...
  // ── Recycle ───────────────────────────────────────────────────────────────
  // Flip coin: heads = put top discard card on top of deck.
  'Recycle': async ({ player, p, consume }) => {
    if (!p.discard.length) { showToast('Discard pile is empty!', true); return; }
    consume();
    const heads = await flipCoin('Recycle: Heads = return top discard card to top of deck');
    if (heads) {
  ...
  // ── Devolution Spray ──────────────────────────────────────────────────────
  // Devolve an evolved Pokémon; the evolution card goes to discard.
  'Devolution Spray': async ({ player, p, consume }) => {
    ...
    const picked = await openCardPicker({ title: 'Devolution Spray', subtitle: 'Choose a Pokémon to devolve', cards: evolved.map(e => ({ name: e.label, images: { small: '' } })), maxSelect: 1 });
  ...
  'Pokémon Breeder': async ({ player, p, card, handIdx }) => {
    ...
    const targets = [{ zone: 'active', card: p.active, idx: null }, ...p.bench.map((b, i) => ({ zone: 'bench', card: b, idx: i }))].filter(t => t.card?.supertype === 'Pokémon');
    ...
    const doBreed = (t) => {
      stage2.damage = t.card.damage || 0; stage2.attachedEnergy = t.card.attachedEnergy || []; stage2.status = null;
      if (t.zone === 'active') p.active = stage2; else p.bench[t.idx] = stage2;
      addLog(`P${player} used Pokémon Breeder — ${t.card.name} → ${stage2.name}!`, true);
      renderAll();
    };
    if (targets.length === 1) { doBreed(targets[0]); return; }
    const tpicked = await openCardPicker({ title: 'Pokémon Breeder — Target', subtitle: 'Choose which Pokémon to evolve', cards: targets.map(t => t.card), maxSelect: 1 });
    if (tpicked && tpicked.length) doBreed(targets[tpicked[0]]);
  },
